...
let padLock = [[None, Some(2), Some(3)], [Some(4), None, Some(6)], [Some(7), Some(8), Some(9)]]

let getVals = ((i, j)): option<int> =>
  padLock[i]->Option.flatMap(arr => arr[j])->Option.flatMap(res => res)

let moves = list{(2, 1), (-2, 1), (2, -1), (-2, -1), (1, 2), (1, -2), (-1, 2), (-1, -2)}

module PairComparator = Id.MakeComparable({
  type t = (int, int)
  let cmp = ((a0, a1), (b0, b1)) =>
...
let posSolutions = (~cache=?, (i, j) as idx) => {
  let solve = () => List.reduce(moves, list{}, (ls, (im, jm)) => {
      let inw = i + im
      let jnw = j + jm
...
let rec findCombinations = (~depth=0, ~cache=?, (i, j) as idx, max_depth) =>
  if depth > max_depth {
    ...
    let children = List.reduce(pos, list{}, (nodes, elem) => {
      let child = findCombinations(~cache?, ~depth=depth + 1, elem, max_depth)
      switch child {
...
->List.forEach(path => {
  print_string("Path >>\n")
  path->List.map(((_, (i, j))) => getVals((i, j)))->List.forEach(n =>
    switch n {
    | Some(n) => Js.log(n)
    | None => ()
    }
